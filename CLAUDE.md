...
- `scripts/e2e.sh` — orquestrador estático (`docker compose up --build --abort-on-container-exit --exit-code-from e2e-playwright`, teardown via `trap`), mesmo padrão de `scripts/frontend-check.sh`. Rodado por um job dedicado no CI (`.github/workflows/ci.yml`, job `e2e`), **bloqueante em todo PR** (sem filtro de path) — deliberadamente enxuto (specs de smoke pro papel admin e um cenário de acesso restrito pro papel viewer, não escala/performance), já que `scripts/merge-when-green.sh` espera todos os check-runs antes de mergear.
...
| `internal/events` | Barramento de eventos in-process (Publish/Subscribe por tipo, não-bloqueante, sem persistência). Doc completa: [docs/go-modules/internal/events/README.md](docs/go-modules/internal/events/README.md). |
| `internal/alerts` | Assina eventos operacionais de `internal/events` (recorder/transmissão parados ou recuperados) e traduz em `Notification` pra todo admin via `notifications.Dispatcher`. Doc completa: [docs/go-modules/internal/alerts/README.md](docs/go-modules/internal/alerts/README.md). |
...
| `internal/notifications/webpush` | Sender Web Push real (Service Worker + Push API) — único canal que entrega notificação de movimento com o app fechado; chaves VAPID estáveis entre reinícios. Doc completa: [docs/go-modules/internal/notifications/webpush/README.md](docs/go-modules/internal/notifications/webpush/README.md). |
