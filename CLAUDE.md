...
`Player` (`components/Player.tsx` — nome antigo `HLSPlayer`) — player de live enxuto, **WebRTC-first com fallback automático pro HLS**: tenta o transporte WebRTC de baixa latência via `lib/webrtc.negotiateWebRTC` (`POST /api/cameras/{id}/webrtc`) e, quando indisponível (`409`/câmera não-H.264), a negociação falha, ou a conexão não fecha em 5 s (watchdog), cai pro `hls.js`. A prop `transport` (o `live_transport` da câmera: `auto`/`webrtc`/`hls`) pula o WebRTC quando `hls`. Autoplay mudo, overlay de loading/tap-to-play, retry com backoff+teto (`components/playerRetry.ts`), zoom (`hooks/usePlayerZoom.ts`) e botão de fullscreen.
...
- `HistoryPage` (`/history/:cameraId(/:recordingId)`) — gravações do dia (calendário via `DatePicker`, default hoje) + player + tira de cards pra trocar de gravação; `:recordingId` na URL pré-seleciona (resolvida via `RecordingsGateway.getRecording`) e a seleção mantém a URL sincronizada — link compartilhável.
- `VideoBrowserPage` (`/recording/:cameraId/:recordingId(/:motionId)`) — reprodução via `RecordingsGateway` (único intermediário com o backend); transição entre chunks com double-buffering (2 `<video>` empilhados, sem tela preta na fronteira — MP4 não-fragmentado, MSE não é opção); barra de controle própria sobreposta aos dois vídeos. `:motionId` monta o clip com lead/trail em torno do evento.
