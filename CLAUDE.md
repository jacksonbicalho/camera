...
`AppSidebar` (ex-`Sidebar`) — sidebar completa do `AppLayout`: seção inferior fixa com ícone de configurações (engrenagem), sino de notificações e usuário; ícone ativo usa `bg-blue-600`; itens injetados pela página corrente via `SidebarItemsProvider` ficam na seção superior.

`Sidebar` — rail de navegação **enxuto** do `Layout` (páginas novas: `LivePage`, `HistoryPage`, `VideoBrowserPage`): só `NavLink`s (Início/Gravações/Relatórios/Configurações), com `id` por item (`sidebar-inicio`…), item ativo em `bg-primary text-on-primary`. **Sem** sino, user-menu, painéis ou contextos pesados — esses vivem no `AppSidebar`. Os dois nunca co-renderizam (uma página usa `AppLayout` **ou** `Layout`).

**Largura do conteúdo nas páginas novas (`Layout`):** classe compartilhada `.page-content` (`styles/base.css`, `mx-auto w-full max-w-[80rem]` — mesmo valor de `max-w-7xl`/o `wide` do `SettingsLayout`) — fonte única da largura padrão, pra garantir a MESMA largura em toda página nova (evita divergência: cada página tinha seu próprio `max-w-*` solto e acabaram desalinhando). `<Layout contentClassName="p-6">` (margem de página) + `<div className="page-content space-y-4">` como wrapper do conteúdo — `LivePage`, `HistoryPage`, `VideoBrowserPage` (as três que usam `Layout`) usam a mesma classe; novas páginas que adotarem `Layout` devem seguir o mesmo, em vez de repetir `max-w-*` na mão. Duas tentativas anteriores foram revertidas: (1) sem cap nenhum (largura 100%), o player (`aspect-video`) ficava alto demais em telas largas; (2) capar o PLAYER por altura de viewport (`max-h`/`aspect-video w-auto`) — o tamanho da caixa não pode depender de estado (senão pula quando os dados carregam) e, mesmo incondicional, deixa vão vazio nas laterais quando a coluna é mais larga que o player. A solução foi manter a largura da COLUNA padronizada (nunca o player individualmente) — `HistoryPage` (que tem o filmstrip abaixo do player, ao contrário de Ao vivo/Reprodução) pode gerar scroll vertical em viewports mais baixas: comportamento normal de página com mais conteúdo, não um bug de layout a "consertar" estreitando ou capando o player.
