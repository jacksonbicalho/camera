...
2. /story cria story JÁ decomposta em tickets (T1..Tn) + 1 cenário funcional por CA em tests/functional/ — AINDA EM develop, SEM branch. GATE G2: nem branch nem red phase antes de `[x] História revisada` — `scripts/await-gate.sh revisao`. Só depois do gate: `git checkout -b <tipo>/<slug> develop`.
