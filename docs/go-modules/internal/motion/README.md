...
- `ffmpeg.go`/`ffmpeg_unix.go`/`ffmpeg_windows.go` — monta os args do pipe
  ...
  plataforma. `core.NeedsRTSPTransport` decide sozinho a única flag extra
  (`-rtsp_transport tcp`, só pra `rtsp`/default); nenhum `capture_type`
  acrescenta timeout de leitura de rede aqui (ver "Decisões e invariantes"
  sobre o histórico do `-rw_timeout` de MJPEG, removido por completo com o
  protocolo).
...
- **MJPEG como protocolo de captura foi removido do projeto inteiro**
  (história `chore/remover-mjpeg-backend`, par de `chore/remover-mjpeg-frontend`
  que já tinha tirado a opção da UI). Até então, `ffmpeg.go` acrescentava
  `-rw_timeout` (em microssegundos) antes de `-i url` só quando
  `capture_type=mjpeg` — MJPEG é HTTP contínuo (`multipart/x-mixed-replace`),
  sem heartbeat como o RTSP tem via RTCP, então sem esse timeout uma câmera
  que parasse de mandar frames sem fechar o socket TCP deixava o processo
  ffmpeg do pipe pendurado pra sempre (o `reconnect_interval` só age depois
  que o processo morre). Esse motivo desapareceu junto com o protocolo: uma
  câmera com `capture_type=mjpeg` remanescente no banco agora cai no
  `default` (rtsp) de todo switch, incluindo aqui — sem captura funcionando
  contra uma URL HTTP, mas também sem o risco de pipe pendurado que motivou
  o timeout.
...
- [internal/capturer](../capturer/README.md) — protocolos de captura "de alto nível" (rtsp/hls).
