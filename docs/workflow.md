...
- **Padrão default: CA verificado por um teste nomeado dentro da suíte nativa da linguagem, anotado `(auto: scripts/check.sh)` — sem script `.sh` dedicado.** `check.sh` já roda `go test ./...` sempre, `frontend-check.sh` quando `frontend/` muda e `yolo-check.sh` quando `services/yolo/` muda — um teste corretamente nomeado/agrupado por CA já fica coberto pela suíte inteira do CA1, então `functional-check.sh` marca `[x]` esses CAs direto, sem rodar nada além do `check.sh` do topo. A garantia real do critério é o teste existir com o nome/estrutura certa E a suíte inteira estar verde — verificável por code review, não por script. Convenção por linguagem:
  - **Frontend**: `describe('CAn: <critério>', () => it('<comportamento>', ...))` dentro de um `*.test.tsx`/`.test.ts`. Rodar um CA isolado localmente: `yarn test -t 'CAn:'` (ou `scripts/frontend-check.sh <arquivo>` pro arquivo inteiro).
  - **Backend Go**: subtestes aninhados via `t.Run`, mapeando história → CA → cenário — `func TestNomeDaHistoria(t *testing.T) { t.Run("CAn: <critério>", func(t *testing.T) { t.Run("<comportamento>", func(t *testing.T) { ... }) }) }`. Rodar um CA isolado: `go test -run 'TestNomeDaHistoria/CAn' ./pkg/...` (cada nível do path do `-run` casa uma regex contra o nível correspondente da árvore de subtestes; bastam os primeiros níveis — `go test` já roda tudo abaixo do prefixo casado. A suíte inteira falha nativamente se o padrão não casar nenhum subteste — `go test` nunca passa em silêncio quando o `TestNomeDaHistoria` não existe).
  - **Backend Python (`services/yolo/`)**: função nomeada `def test_caN_<comportamento>(): ...` (prefixo `test_caN_` greppável, mesmo espírito do `t.Run`/`describe` — pytest não tem subteste nativo tão ergonômico quanto Go/JS). Rodar um CA isolado: `pytest -k caN` (via `scripts/yolo-check.sh`, que aceita args extras do mesmo jeito que `frontend-check.sh <arquivo>`: `bash scripts/yolo-check.sh -k caN`).

  Existe porque um script `.sh` dedicado por CA só reproduzia o mesmo `<runner> -k/-t/-run '<padrão>'` + verificação de "achou e passou" (workaround pro test runner não falhar quando o padrão não casa nada) repetido a cada CA — trabalho e arquivo redundantes quando o próprio nome do teste já é o cenário funcional. Convenção adotada nas histórias `refactor/frontend-testes-ca-describe` (`work_progress/analysis/202607232012_frontend-testes-ca-describe.md`, origem — só frontend) e generalizada pra Go/Python em `fix/fine-tuning-yolo-gpu`.
- **`tests/functional/caNN_<slug>.sh` (versionado) continua existindo para o que genuinamente não é um teste automatizado de unidade/integração da própria suíte** — ex.: cenários que sobem o binário completo, batem HTTP de fora, ou orquestram múltiplos processos (Docker, e2e). Convenção: executável, exit 0 = critério atendido. Ver template em `tests/functional/ca2_exemplo.sh.template`.
- `scripts/functional-check.sh`: roda `check.sh` (marca CA1), marca `[x]` todo CA anotado `(auto: scripts/check.sh)` (coberto pela suíte, ver acima) e roda cada `tests/functional/caNN_*.sh` referenciado na story pros CAs que ainda usam script dedicado, marcando `[x]` ao passar. Falhou → CA fica `[]` e o script sai com erro (driver corrige e re-roda; se a correção mexer em lógica, o ticket volta ao loop de review).
...
| `record-review.sh <Tn> <iterações>` | Claude | Registra o veredito `APPROVED` do subagent `code-reviewer` na seção `## Code Review`, marca o Status do ticket na tabela, e abre `Review: APPROVED` quando todos os tickets estão `[x]`. **Envie o resultado do subagent via stdin** (`printf '%s\n' "$review_result" \| scripts/record-review.sh Tn N`) — o corpo (Issues/Observações) é indentado e persistido logo abaixo da linha de veredito, não só o "APPROVED (N iterações)"; sem stdin, grava só a linha (compatível com uso antigo). |
