      ...
      vi.fn((url: string) => {
        if (url === `/api/cameras/${cameraId}/motion?date=${dateStr}`) {
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ events: [{ id: 42, time: isoTime, score: 0.8 }] }),
          })
        }
    ...
    vi.stubGlobal(
      ...
      vi.fn((url: string) => {
        if (url.includes('/motion?date='))
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ events: [{ id: -3, time: isoTime, kind: 'state' }] }),
          })
        if (url.includes('/recordings?date='))
          return Promise.resolve({
            ok: true,
    ...
    vi.stubGlobal(
      ...
      vi.fn((url: string) => {
        if (url.includes('/motion?date='))
          return Promise.resolve({
            ok: true,
            json: () => Promise.resolve({ events: [{ id: 42, time: isoTime }] }),
          })
        if (url.includes('/recordings?date='))
