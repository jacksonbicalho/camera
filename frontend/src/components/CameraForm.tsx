...
export default function CameraForm({ initial, prefillRtsp, prefillName, onSave, onCancel, saving }: CameraFormProps) {
  ...
  const selectClass = "w-full bg-background border border-border rounded px-3 py-1.5 text-sm text-foreground focus:outline-none focus:border-primary"
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <Label htmlFor="camera-form-name" className={labelClass}>Nome</Label>
          <Input
            id="camera-form-name"
            value={form.name}
        ...
        <div className="sm:col-span-1">
          <Label htmlFor="camera-form-rtsp-url" className={labelClass}>RTSP URL</Label>
          <Input
        ...
        <div className="sm:col-span-2">
          <Label htmlFor="camera-live-transport" className={labelClass}>Transporte do ao-vivo</Label>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">WebRTC entrega o ao-vivo com latência abaixo de 1s (exige H.264 no stream principal). "Detectar" verifica o codec para recomendar o transporte.</p>
        </div>
      </div>
      ...
      <div className="border-t border-border pt-3">
        ...
        {showAdvanced && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
            <div className="sm:col-span-2">
              <Label htmlFor="camera-motion-rtsp-url" className={labelClass}>RTSP URL da detecção de movimento (substream)</Label>
              <div className="flex gap-2">
                <Input
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Opcional. Vazio = usa o stream principal. "Detectar" tenta descobrir o substream a partir da URL principal (menor resolução) — reduz muito o custo de CPU da detecção; o snapshot do evento sai nessa resolução.</p>
            </div>
            <div>
              <Label htmlFor="camera-form-chunk-duration" className={labelClass}>Duração do chunk</Label>
              <Input
            ...
            <div>
              <Label htmlFor="camera-form-reconnect-interval" className={labelClass}>Intervalo de reconexão</Label>
              <Input
            ...
            <div>
              <Label htmlFor="camera-form-video-codec" className={labelClass}>Codec de vídeo</Label>
              <select id="camera-form-video-codec" value={form.video_codec} onChange={e => set('video_codec', e.target.value)} className={selectClass}>
            ...
            <div>
              <Label htmlFor="camera-form-has-audio" className={labelClass}>Áudio</Label>
              <select id="camera-form-has-audio" value={form.has_audio} onChange={e => set('has_audio', e.target.value)} className={selectClass}>
            ...
            <div>
              <Label htmlFor="camera-form-resolution" className={labelClass}>Resolução</Label>
              <select id="camera-form-resolution" value={form.resolution} onChange={e => set('resolution', e.target.value)} className={selectClass}>
                {RESOLUTIONS.map(r => (
            ...
            <div>
              <Label htmlFor="camera-form-record-video-mode" className={labelClass}>Modo de gravação</Label>
              <select id="camera-form-record-video-mode" value={form.record_video_mode} onChange={e => set('record_video_mode', e.target.value)} className={selectClass}>
            ...
            <div className="sm:col-span-2 mt-1">
              <p className="text-xs font-medium text-faint">HLS (ao vivo)</p>
            </div>
            <div>
              <Label htmlFor="camera-form-hls-video-mode" className={labelClass}>Modo de vídeo HLS</Label>
              <select id="camera-form-hls-video-mode" value={form.hls_video_mode} onChange={e => set('hls_video_mode', e.target.value)} className={selectClass}>
            ...
            <div>
              <Label htmlFor="camera-form-hls-segment-seconds" className={labelClass}>Duração do segmento (s)</Label>
              <select
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Cada segmento de vídeo ao vivo tem essa duração. Valores menores reduzem a latência, mas aumentam o processamento.</p>
            </div>

            <div>
              <Label htmlFor="camera-form-hls-list-size" className={labelClass}>Janela de reprodução (segmentos)</Label>
              <select
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Quantidade de segmentos mantidos na playlist ao vivo. A latência aproximada é duração × janela (padrão ≈ 10 s).</p>
            </div>

            <div>
              <Label htmlFor="camera-form-hls-dvr-seconds" className={labelClass}>Retenção DVR (s)</Label>
              <Input
                ...
                placeholder="0"
              />
              <p className="text-xs text-muted-foreground mt-0.5">Tempo máximo, em segundos, que o histórico ao vivo fica disponível para consulta. Permite buscar eventos recentes sem gravação em disco. <span className="text-muted-foreground">0 = desativado.</span></p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="border-t border-border pt-3">
        <p className="text-xs font-medium text-muted-foreground mb-3">Gravação</p>
        ...
        {!form.recording_enabled && (
          <p className="text-xs text-muted-foreground mt-1">HLS e detecção de movimento continuam funcionando</p>
        )}
      </div>
