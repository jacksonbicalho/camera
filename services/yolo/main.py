...
# Serializa qualquer uso da GPU (inferência /analyze, treino em background de
# _run_finetune) — evita OOM por treino e inferência concorrentes disputando
# a mesma VRAM. Handlers HTTP síncronos
...
def _run_finetune(job_id: str, req: FinetuneRequest):
    ...
    try:
        ...
        # CA4: adquire bloqueante — ao contrário do handler HTTP síncrono
        # (/analyze), esta é uma thread de treino em background, já
        # assíncrona do ponto de vista do cliente (monitorada por polling
