...
3. **Confirme o entendimento** devolvendo um resumo curto (bullets) dos pontos-gate, sem reabrir discussão e sem começar tarefa nenhuma:
   ...
   - **G2:** `/story` escreve, ainda em `develop` (sem branch), a story já decomposta em tickets (T1..Tn) + todo CA verificado por um teste nomeado na suíte permanente correspondente (nunca script `.sh` dedicado a um CA). Não implementar (nem red phase) antes de `[x] História revisada`. Só depois do gate: `git checkout -b <tipo>/<slug> develop`.
