...
// has_motion em internal/storage/cleaner.go já usa).
...
// TestMomentsNoLongerIncludesStateTransitions cobre a história
// chore/remover-classificacao-estados-backend — handleMoments (GET /api/moments) para de
// mesclar transições de camera_state_history no feed unificado. Usa SQL bruto (sem
// db.CreateStateClassifier/stateclass) pra sobreviver ao ticket que deleta esse pacote,
// mesmo raciocínio de TestMotionEventsNoLongerMergesStateTransitions em server_test.go.
func TestMomentsNoLongerIncludesStateTransitions(t *testing.T) {
	t.Run("CA4: GET /api/moments não inclui mais transições de estado (classificação de estado removida)", func(t *testing.T) {
		database := openServerTestDB(t)
		if _, err := db.CreateUser(database, "admin", "pw", "admin", false); err != nil {
			t.Fatal(err)
		}
		...
		day := time.Date(2026, 6, 23, 0, 0, 0, 0, time.UTC)
		if err := db.InsertMotionEvent(database, db.MotionEvent{
			CameraID: "cam1", OccurredAt: day.Add(8 * time.Hour), Score: 0.5, FramePath: "a.jpg",
		}); err != nil {
			t.Fatal(err)
		}
		...
		if _, err := database.Exec(
			...
			cid, "aberto", 0.9, "/recordings/state_history/1/x.jpg", "2026-06-23 09:00:00",
		); err != nil {
			t.Fatal(err)
		}
		...
		if len(resp.Moments) != 1 {
			t.Fatalf("esperado 1 momento (só movimento, sem estado), got %d: %+v", len(resp.Moments), resp.Moments)
		}
		for _, m := range resp.Moments {
			if m["kind"] == "state" {
				t.Fatalf("momento kind=state não deveria mais aparecer: %+v", resp.Moments)
			}
		}
	})
}
