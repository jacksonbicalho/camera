...
func TestSnapshot(t *testing.T) {
	t.Run("CA4: monta os mesmos argumentos ffmpeg que takeSnapshot produzia (1 frame JPEG via RTSP)", func(t *testing.T) {
		fake := &fakeExecutor{out: []byte("jpeg-bytes")}
		data, err := rtsp.Snapshot(context.Background(), "rtsp://cam/stream", fake)
		if err != nil {
	...
	t.Run("CA4: propaga erro do executor", func(t *testing.T) {
		...
		if _, err := rtsp.Snapshot(context.Background(), "rtsp://cam/stream", fake); err == nil {
